-- 🚨 DIAGNÓSTICO URGENTE - GRUPOS EN POLANCO (CORREGIDO)
...
FROM schedules s
LEFT JOIN students st ON st.schedule_id = s.id
GROUP BY s.id, s.classroom, s.day_of_week
ORDER BY s.day_of_week, s.classroom;

-- 6. VERIFICAR ESTUDIANTES HUÉRFANOS (sin schedule válido)
SELECT 
    st.id,
    st.name,
    st.last_name,
    st.schedule_id,
    'ESTUDIANTE SIN HORARIO VÁLIDO' as problema
FROM students st
LEFT JOIN schedules s ON st.schedule_id = s.id
WHERE s.id IS NULL AND st.schedule_id IS NOT NULL;
